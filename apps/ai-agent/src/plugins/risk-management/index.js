...
exports.defaultRiskConfig = void 0;
exports.createRiskManagementPlugin = createRiskManagementPlugin;
const agent_1 = require("@iqai/agent");
const index_js_1 = require("../../utils/index.js");
const logger = new index_js_1.Logger('RiskManager');
class RiskManagerService {
    ...
    checkDailyReset() {
        ...
        if (now.getDate() !== this.lastResetDate.getDate()) {
            logger.info(`Daily reset: Previous day P&L: ${(0, index_js_1.formatCurrency)(this.dailyPnL)}`);
            this.dailyPnL = 0;
            this.lastResetDate = now;
        }
    }
    // Evaluate if a trading signal should be executed based on risk parameters
    evaluateSignal(signal, currentRisk) {
        this.checkDailyReset();
        // Check if signal meets minimum confidence threshold
        if (!(0, index_js_1.shouldExecuteTrade)(signal, 70)) {
            return {
                approved: false,
        ...
        if (this.dailyPnL < -maxDailyLossAmount) {
            return {
                approved: false,
                reason: `Daily loss limit exceeded: ${(0, index_js_1.formatCurrency)(Math.abs(this.dailyPnL))} / ${(0, index_js_1.formatCurrency)(maxDailyLossAmount)}`
            };
        }
        ...
        const basePositionSize = (0, index_js_1.calculatePositionSize)(this.config.portfolioValue, currentRisk.maxPositionSize, signal.confidence);
        ...
        if (currentDrawdown > 0.1) { // If drawdown > 10%, reduce position sizes
            ...
            logger.warn(`Position size reduced due to drawdown: ${(0, index_js_1.formatPercentage)(currentDrawdown)}`);
        }
        ...
        if (currentDrawdown > this.config.emergencyStopLoss / 100) {
            return {
                approved: false,
                reason: `EMERGENCY STOP: Portfolio drawdown ${(0, index_js_1.formatPercentage)(currentDrawdown)} exceeds limit ${(0, index_js_1.formatPercentage)(this.config.emergencyStopLoss / 100)}`
            };
        }
        return {
            approved: true,
            reason: `Signal approved: ${signal.confidence}% confidence, ${(0, index_js_1.formatCurrency)(adjustedPositionSize)} position`,
    ...
    recordTrade(trade) {
        ...
        logger.info(`Trade recorded: ${trade.side.toUpperCase()} ${trade.amount} ${trade.symbol} at ${(0, index_js_1.formatCurrency)(trade.price)}`);
    }
    // Update trade status and P&L
    updateTrade(tradeId, status, exitPrice) {
        ...
        if (status === 'filled' && exitPrice) {
            ...
            this.dailyPnL += pnl;
            this.totalPnL += pnl;
            logger.info(`Trade completed: ${(0, index_js_1.formatCurrency)(pnl)} P&L, Daily: ${(0, index_js_1.formatCurrency)(this.dailyPnL)}, Total: ${(0, index_js_1.formatCurrency)(this.totalPnL)}`);
        }
    }
    ...
    optimizePortfolio(signals, currentRisk) {
        // Sort signals by confidence and urgency
        const sortedSignals = signals.sort((a, b) => {
            if (a.urgency === 'immediate' && b.urgency !== 'immediate')
                return -1;
            if (b.urgency === 'immediate' && a.urgency !== 'immediate')
                return 1;
        ...
        for (const signal of sortedSignals) {
            ...
            if (evaluation.approved && evaluation.adjustedPositionSize) {
                if (evaluation.adjustedPositionSize <= remainingCapital) {
                    const optimizedSignal = Object.assign(Object.assign({}, signal), { positionSize: evaluation.adjustedPositionSize });
...
function createRiskManagementPlugin(config) {
        ...
        actions: [
            {
                ...
                handler: (opts) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    try {
                        ...
                        const events = ((_a = opts.runtime) === null || _a === void 0 ? void 0 : _a.marketEvents) || [];
                        const signalsToEvaluate = events
                            .filter((e) => e.tradingSignal && e.tradingSignal.action !== 'hold')
                            .map((e) => e.tradingSignal)
                        ...
                        for (const signal of signalsToEvaluate) {
                            ...
                            if (evaluation.adjustedPositionSize) {
                                response += `Position Size: ${(0, index_js_1.formatCurrency)(evaluation.adjustedPositionSize)}\n`;
                            }
                        ...
                        response += `Portfolio Value: ${(0, index_js_1.formatCurrency)(metrics.portfolioValue)}\n`;
                        response += `Daily P&L: ${(0, index_js_1.formatCurrency)(metrics.dailyPnL)}\n`;
                        response += `Total P&L: ${(0, index_js_1.formatCurrency)(metrics.totalPnL)}\n`;
                        response += `Current Drawdown: ${(0, index_js_1.formatPercentage)(metrics.currentDrawdown)}\n`;
            ...
            },
            {
                ...
                handler: (opts) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    try {
                        ...
                        response += `• Portfolio Value: ${(0, index_js_1.formatCurrency)(metrics.portfolioValue)}\n`;
                        response += `• Daily P&L: ${(0, index_js_1.formatCurrency)(metrics.dailyPnL)} (${(0, index_js_1.formatPercentage)(metrics.dailyLossPercentage)})\n`;
                        response += `• Total P&L: ${(0, index_js_1.formatCurrency)(metrics.totalPnL)}\n`;
                        response += `• Current Drawdown: ${(0, index_js_1.formatPercentage)(metrics.currentDrawdown)}\n\n`;
                    ...
                    try {
                        ...
                        if (optimizedSignals.length > 0) {
                            ...
                            optimizedSignals.forEach((signal, index) => {
                                ...
                                response += `   Confidence: ${signal.confidence}%\n`;
                                response += `   Position: ${(0, index_js_1.formatCurrency)(signal.positionSize || 0)}\n`;
                                response += `   Urgency: ${signal.urgency.toUpperCase()}\n\n`;
                            });
                            const totalAllocation = optimizedSignals.reduce((sum, s) => sum + (s.positionSize || 0), 0);
                            response += `**Total Allocation: ${(0, index_js_1.formatCurrency)(totalAllocation)}**\n`;
                            response += `**Portfolio Utilization: ${(0, index_js_1.formatPercentage)(totalAllocation / config.portfolioValue)}**\n`;
                        }
                        else {
